...
public class Bank {
	...
	public TransactionReceipt getBalance(TransactionTicket ticket) {
		...
		if (index == -1) {
			...
			TransactionReceipt receipt = filler.getBalance(ticket);
			return receipt;
		}
	}
	...
	public TransactionReceipt makeWithdraw(TransactionTicket ticket) {
		...
		if (index == -1) {
			...
			Account filler = getAccount(index);
			TransactionReceipt receipt = filler.makeWithdraw(ticket);
			return receipt;
		}
	...
	The output is a receipt which is made depending on if the transaction went through.
	 */
	public TransactionReceipt newAcct(TransactionTicket ticket, Depositor info, String type) {
		int account = ticket.getAcctNum();
		...
		if (ssn.length() == 9) {
			if (index != -1) {
				...
				if (type.equals("Savings") || type.equals("Checking")) {
					...
					Account newAccount = new Account(info, account, type, 0, present, true);
					...
					if ( !(cdBalance >= 0) ) {
						...
						if ( termNext == 6 || termNext == 12 || termNext == 18 || termNext == 24 ) {
							...
							Account newAccount = new Account(info, account, type, cdBalance, date, true);
							addAccount(newAccount);
							newAccount.addTransaction(receipt);
